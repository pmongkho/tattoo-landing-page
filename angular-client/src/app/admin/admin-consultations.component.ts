...
<tbody><tr *ngFor="let c of consultations" class="border-t border-zinc-800"><td>{{ c.name }}</td><td>{{ c.phoneNumber }}</td><td>{{ c.email }}</td><td>{{ c.style }}</td><td>{{ c.placement }}</td><td>{{ c.size }}</td><td>{{ c.budget }}</td><td>{{ c.preferredDays.map(d => d.day).join(', ') }}</td><td>{{ c.description }}</td><td>
